...
<svg width="%d" height="%d" viewBox="%d %d %d %d"
  ...
  <path fill=\"none\" stroke=\"black\" d=\"""" % (sx, sy, self.minx, self.miny, sx, sy)
		self.pos = self.coords[0]
		last_jump = False
		...
		for stitch in self.coords[0:]:
			if stitch.jump:
				...
				if last_jump:
					...
					self.str += "  <path fill=\"none\" stroke=\"black\" d=\"M %d %d" % (stitch.x, stitch.y)
					last_jump = False
